...
- 상태: **후보 자산 제출됨. 아트 디렉션 승인 미발행, 인간 게이트 미발행**
...
서빙. 그리고 **금지 내용 판정과 톤 판정도 닫지 않는다.** 아래 5절과 6절이 그 이유를 기록한다.
...
미결이며 `A-30`이 판정 소유자다. 자산 상태는 **후보(CANDIDATE) / 제출됨(SUBMITTED)**이고,
`A-30`의 아트 디렉션 승인, `A-50`의 독립 검토, `USER`의 아트 방향 사인오프가 모두 기록되기
전까지 어떤 산출물도 확정 자산이 아니다.

## 6. 시각 검토 상태 — PENDING이며 PASS가 아니다

기록해야 할 사실은 두 가지이고, 둘을 섞지 않는다.

1. 공급자의 원본 출력물은 생성 시점에 화면에 표시되었고, 그 시점에 Codex가 관측한 금지 내용은
   없었다.
2. **최종 처리본 파일의 브라우저 시각 QA는 재실행하지 못했다.** Windows view-image 샌드박스가
   실패해 처리 후의 WebP와 PNG를 다시 표시할 수 없었다.

따라서 사람에 의한 시각 검토와 금지 내용 검토는 **PENDING**이다. **PASS가 아니다.** 4절의
측정값은 픽셀 통계일 뿐이며 `AC-010`의 금지 내용 판정과 `AC-011`의 톤 판정을 대신하지 않는다.
두 판정은 `A-30`의 원본 해상도·400% 확대 체크리스트 판정과 `A-50`의 독립 재판정으로만 닫힌다.
...
- `A-50` 독립 검토
- `USER` 아트 방향 사인오프
- 처리본에 대한 사람의 시각 검토(원본 해상도 및 400% 확대)
- 금지 내용 체크리스트 독립 판정 (`AC-010`)
- 브리프 톤 기준 대비 판정 (`AC-011`)
- 권리·학습 허용 판정 (`AC-009`)
- 기존 표면 불투명도로 합성한 명암비 검증 (`AC-007`)
- `Artifact Contract`와 `Handoff Packet` 제출 (이 시점에는 아직 만들지 않았다)

후속 통합 유닛의 착수 조건은 위 사인오프가 모두 기록되는 것이다.
